    ...
    if (provider && authCode && !isAuthenticating) {
      ...
      const handleGithubAuth = async () => {
        try {
          ...
          const requestUrl = `${backend_uri}/api/v1/auth/github/register`;
          ...
          if (!response.ok) {
            ...
            if (response.status === 500) {
              setAuthError(
                "Server error (500): The authentication server encountered an error. Please try again later or contact support."
              );
            } else {
              setAuthError(
          ...
          if (data && data.user.id) {
            ...
            setTimeout(() => {
              ...
              if (storedUser) {
                ...
                if (parsedUser) {
                  router.push(
                    `/dashboard/projects?provider=${provider}&authcode=${authCode}`
                  );
                }
              } else {
                setDebugInfo(
        ...
        <div className="w-full">
          ...
          >
            ...
            <div className="w-full flex justify-center items-center">
              <span className="text-white manrope font-normal text-sm">

...
const OTPVerificationPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await verificationService.verifyOtp(userEmail, otpCode)
  ...
  const handleResend = async () => {
    ...
    setError(null)
    try {
      await verificationService.sendOtp(userEmail)
      setResent(true)
    ...
    <main className="min-h-screen bg-white flex items-center justify-center">
      <div className="w-full h-screen grid lg:grid-cols-2 items-stretch">
        ...
        <div className="relative hidden lg:flex overflow-hidden bg-black p-10 flex-col justify-between border-r border-gray-200">
          ...
          <div className="relative z-10 mt-auto">
            <div className="border border-zinc-800 bg-zinc-900/40 backdrop-blur-sm p-5">
              ...
              <p className="mt-3 text-sm font-semibold text-white font-mono truncate">{userEmail}</p>
        ...
        <div className="bg-white p-8 lg:p-12 flex flex-col justify-center max-w-lg mx-auto w-full">
          <Link href="/auth/signup" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-black transition-colors mb-8 w-fit">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500 font-aeonik-light mt-2">
              Enter the 6-digit code we sent to <span className="font-medium text-black">{userEmail}</span>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <button type="submit" disabled={loading || otp.join('').length !== 6} className="btn-primary flex items-center justify-center gap-2">
              {loading ? <><Loader className="h-4 w-4 animate-spin" /> Verifying...</> : 'Verify Email'}
            </button>
          </form>

          <div className="mt-6 flex items-center justify-center gap-2 text-sm">
            <span className="text-gray-500 font-aeonik-light">Didn&apos;t receive the code?</span>
            <button onClick={handleResend} disabled={resending} className="font-semibold text-black hover:underline disabled:opacity-50 disabled:cursor-not-allowed">
